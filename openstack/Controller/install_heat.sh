...
LOCALHOSTNAME=$1
HEATDOMAINID=$(openstack domain list | grep -i heat | awk '{print $2}')
...
### Install
	printf " ### Install Packages "
		if apt-get install heat-api heat-api-cfn heat-engine python-heatclient -y >> /tmp/os_logs/heat.log 2>&1; then
			...
			printf $red " --> Could not install Heat Packages - see /tmp/os_logs/heat.log"
		fi				
...
printf " ### Populate Heat Database"
		if su -s /bin/sh -c "heat-manage db_sync" heat >> /tmp/os_logs/heat.log 2>&1 >> /tmp/os_logs/heat.log 2>&1; then
			...
			printf $red " --> Could not populate Heat Database - see /tmp/os_logs/heat.log"		
		fi
		
### Restart Glance-Api
printf " ### Restart Glance Services \n"
	if service heat-api restart >> /tmp/os_logs/glance.log 2>&1; 		then printf $green " --> Restart Heat-Api done"; 			else printf $red " --> Could not restart Heat-Api Service - see /tmp/os_logs/heat.log"; fi
	if service heat-api-cfn restart >> /tmp/os_logs/glance.log 2>&1; then printf $green " --> Restart Heat-api-cfn done"; 	else printf $red " --> Could not restart Heat-api-cfn - see /tmp/os_logs/heat.log"; fi		
	if service heat-engine restart >> /tmp/os_logs/glance.log 2>&1; then printf $green " --> Restart Heat-engine  done"; 	else printf $red " --> Could not restart Heat-engine  Service - see /tmp/os_logs/heat.log"; fi		
