...
printf " ### Configure MariaDB\n" | tee -a $LOGFILE
	...
	if (cp ${INSTALLPATH}/base/configs/my.cnf /etc/mysql/my.cnf) >> $LOGFILE 2>&1; then printf " --> SUCCESSFUL - copied config file \n"; else printf " --> ERROR - could not copy config file - Logfile: $LOGFILE \n" | tee -a $LOGFILE; fi 
	sed -i '/bind-address*/c\bind-address            = '$CONTROLLERIP /etc/mysql/my.cnf	
...
#Not needed in Liberty
if [ $OSVERSION = "mitaka" ]; then 
