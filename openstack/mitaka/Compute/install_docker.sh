...
printf "\n\n #### Start Docker Installation \n"
	...
	if (git checkout -b stable/mitaka origin/stable/mitaka) >> /tmp/os_logs/nova_docker.log 2>&1; then
		printf " --> Checked out at origin/stable/mitaka \n"
	else
		printf " ERROR  --> Could not checke out at origin/stable/mitaka - see /tmp/os_logs/nova_docker.log\n"
	fi
