...
DEFAULTDOMAINID=$(openstack --os-username admin --os-password Password123! --os-project-name admin --os-user-domain-name default --os-project-domain-name default --os-auth-url $OS_URL --os-identity-api-version 3 domain list | grep -i default | awk '{print $2}')
HEATDOMAINID=$(openstack --os-username admin --os-password Password123! --os-project-name admin --os-user-domain-name default --os-project-domain-name default --os-auth-url $OS_URL --os-identity-api-version 3 domain list | grep -i heat | awk '{print $2}')
...
printf " ### Configure Heat\n" | tee -a $LOGFILE
	if (cp ${INSTALLPATH}/configs/heat.conf /etc/heat/heat.conf) >> $LOGFILE 2>&1; then printf " --> SUCCESSFUL - copied heat.conf \n"; else printf " --> ERROR - could not copy heat.conf  - Logfile: $LOGFILE \n" | tee -a $LOGFILE; fi
