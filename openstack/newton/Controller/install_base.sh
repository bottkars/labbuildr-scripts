...
if (apt-get install software-properties-common -y && add-apt-repository cloud-archive:newton -y ) >> $LOGFILE 2>&1; then
	printf " --> SUCCESSFUL added Newton Repository \n"; else printf " --> ERROR - could not add Newton Repository - Logfile: $LOGFILE \n" | tee -a $LOGFILE; fi
...
${INSTALLPATH}/install_nova.sh $LOGFILE $CONTROLLERNAME $CONTROLLERIP
