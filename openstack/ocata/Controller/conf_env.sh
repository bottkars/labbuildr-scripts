...
printf " ## Configure Admin related Network Settings \n"	
	...
	if ($ADMINBASECOMMAND network create --project admin Admin_Net) >> $LOGFILE 2>&1; 		then printf " --> SUCCESSFUL - Created Network Admin_Net for Tenant admin \n"; 	else printf " --> ERROR - Could not create Network Admin_Net for Tenant admin - see $LOGFILE \n" | tee -a $LOGFILE; fi
	if ($ADMINBASECOMMAND subnet create --project admin --gateway 172.16.0.1 --allocation-pool start=172.16.0.11,end=172.16.0.250 --dns-nameserver 8.8.8.8 --subnet-range 172.16.0.0/24 --network Admin_Net Admin_SN  ) >> $LOGFILE 2>&1; 		then printf " --> SUCCESSFUL - Created Subnet Admin_SN for Tenant admin \n"; 	else printf " --> ERROR - Could not create Subnet Admin_SN for Tenant admin - see $LOGFILE \n" | tee -a $LOGFILE; fi
...
printf " ## Create Router and add it to all networks \n"	
	...
	if ($ADMINBASECOMMAND router add subnet OSRouter Admin_SN ) >> $LOGFILE 2>&1; 		then printf " --> SUCCESSFUL - Added Router Interface to Admin_SN \n"; 	else printf " --> ERROR - Could not add Router Interface to Production_SN - see $LOGFILE \n" | tee -a $LOGFILE; fi
...
printf " ## Get Cirros and create Image \n"	
	...
	if ($ADMINBASECOMMAND flavor create --id 1 --ram 256 --disk 1 --vcpus 1 m1.tiny) >> $LOGFILE 2>&1; 		then printf " --> SUCCESSFUL - Created Flavor m1.tiny \n"; 	else printf " --> ERROR - Could not create Flavor m1.tiny - see $LOGFILE \n" | tee -a $LOGFILE; fi
	if ($ADMINBASECOMMAND flavor create --id 2 --ram 512 --disk 1 --vcpus 1 m1.small) >> $LOGFILE 2>&1; 		then printf " --> SUCCESSFUL - Created Flavor m1.small \n"; 	else printf " --> ERROR - Could not create Flavor m1.small - see $LOGFILE \n" | tee -a $LOGFILE; fi
	if ($ADMINBASECOMMAND flavor create --id 3 --ram 1024 --disk 1 --vcpus 1 m1.medium) >> $LOGFILE 2>&1; 		then printf " --> SUCCESSFUL - Created Flavor m1.medium \n"; 	else printf " --> ERROR - Could not create Flavor m1.medium - see $LOGFILE \n" | tee -a $LOGFILE; fi
	if ($ADMINBASECOMMAND flavor create --id 4 --ram 2048 --disk 1 --vcpus 2 m1.large) >> $LOGFILE 2>&1; 		then printf " --> SUCCESSFUL - Created Flavor m1.large \n"; 	else printf " --> ERROR - Could not create Flavor m1.large - see $LOGFILE \n" | tee -a $LOGFILE; fi
	if ($ADMINBASECOMMAND flavor create --id 5 --ram 4096 --disk 1 --vcpus 2 m1.xlarge) >> $LOGFILE 2>&1; 		then printf " --> SUCCESSFUL - Created Flavor m1.xlarge \n"; 	else printf " --> ERROR - Could not create Flavor m1.xlarge - see $LOGFILE \n" | tee -a $LOGFILE; fi
		...
		then
			printf ' ### Create Volume Types for ScaleIO_Thin / ScaleIO_Thick Provisioning \n'
				...
				printf ' ### Set Properties for volume Types ScaleIO_Thin / ScaleIO_Thick \n'
					...
					if ($ADMINBASECOMMAND volume type set --property volume_backend_name=scaleio ScaleIO_Thick) >> $LOGFILE 2>&1; then printf " --> SUCCESSFUL - Set volume_backend_name=scaleio for volume type ScaleIO_Thick \n"; 	else printf " --> ERROR - Could not set olume_backend_name=scaleio for volume type ScaleIO_Thick - see $LOGFILE \n" | tee -a $LOGFILE; fi
		fi
...
if [[ $CINDERBACKENDS == *"unity"* ]]
		then
			printf ' ### Create Volume Types for Unity_iSCSI_Thin / Unity_iSCSI_Thick Provisioning \n'
				...
				printf ' ### Set Properties for volume Types Unity_iSCSI_Thin / Unity_iSCSI_Thick \n'
