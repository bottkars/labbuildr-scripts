...
printf "
  -----------------------------------------
 | #### Started Glance Installation ##### |
  -----------------------------------------\n\n" | tee -a $LOGFILE
 
printf " ### Install Packages\n" | tee -a $LOGFILE
	if (apt-get install glance -y) >> $LOGFILE 2>&1; then printf " --> SUCCESSFUL - installed Glance Packages\n"; else printf " --> ERROR - could not install Glance Packages - Logfile: $LOGFILE \n" | tee -a $LOGFILE; fi

printf " ### Configure Glance \n" | tee -a $LOGFILE
	if (cp ${INSTALLPATH}/configs/glance-api.conf /etc/glance/glance-api.conf) >> $LOGFILE 2>&1; then printf " --> SUCCESSFUL - Copied glance-api.conf file \n"; else printf " --> ERROR - Could not copy glance-api.conf file - Logfile: $LOGFILE \n" | tee -a $LOGFILE; fi 
	if (cp ${INSTALLPATH}/configs/glance-registry.conf /etc/glance/glance-registry.conf) >> $LOGFILE 2>&1; then printf " --> SUCCESSFUL - Copied glance-registry.conf file \n"; else printf " --> ERROR - Could not copy glance-registry.conf file - Logfile: $LOGFILE \n" | tee -a $LOGFILE; fi 
