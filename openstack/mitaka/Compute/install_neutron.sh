...
### Install
	printf " ### Install Packages "
		if apt-get install neutron-plugin-linuxbridge-agent -y >> /tmp/os_logs/neutron.log 2>&1; then
					printf $green " --> done"
		else
...
#Copy Predefined Configs
	printf " ### Configure Neutron \n"
		...
		sed -i '/auth_url = http*/c\auth_url = http://'$CONTROLLERNAME':35357' /etc/neutron/neutron.conf
