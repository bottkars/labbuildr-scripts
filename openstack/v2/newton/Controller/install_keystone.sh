...
printf " ### Create Keystone Domains \n"
	if ($BASECOMMAND domain create default --description "Default" ) >> $LOGFILE 2>&1; 		then printf " --> SUCCESSFUL Created Keystone Domain Default \n"; 	else printf " --> ERROR - Could not create Keystone Domain Default - see $LOGFILE \n" | tee -a $LOGFILE; fi	
...
printf " ### Create Endpoints \n"
	#Keystone
	if ($BASECOMMAND endpoint create --region RegionOne identity public http://$CONTROLLERNAME:5000/v3) >> $LOGFILE 2>&1; 	then printf " --> SUCCESSFUL Created Keystone public endpoint\n"; 	else printf " --> ERROR - Could not create Keystone public endpoint - see $LOGFILE \n" | tee -a $LOGFILE; fi
	if ($BASECOMMAND endpoint create --region RegionOne identity internal http://$CONTROLLERNAME:5000/v3) >> $LOGFILE 2>&1; 	then printf " --> SUCCESSFUL Created Keystone internal endpoint\n"; else printf " --> ERROR - Could not create Keystone internal endpoint - see $LOGFILE \n" | tee -a $LOGFILE; fi
	if ($BASECOMMAND endpoint create --region RegionOne identity admin http://$CONTROLLERNAME:35357/v3) >> $LOGFILE 2>&1; then printf " --> SUCCESSFUL Created Keystone admin endpoint\n"; 	else printf " --> ERROR - Could not create Keystone admin endpoint - see $LOGFILE \n" | tee -a $LOGFILE; fi
	#Glance
