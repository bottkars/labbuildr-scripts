...
printf "\n\n #### Start Keystone Installation \n"
	...
	printf " ### Install Packages "
		if apt-get install keystone apache2 libapache2-mod-wsgi -y >> /tmp/os_logs/keystone.log 2>&1; then
			...
			printf $red " ERROR --> Could not install Keystone Packages - see /tmp/os_logs/keystone.log"
		fi		
...
### Populate Database
	printf " ### Populate Keystone Database "
		if su -s /bin/sh -c "keystone-manage db_sync" keystone >> /tmp/os_logs/keystone.log 2>&1; then
			...
			printf $red "  ERROR --> Could not populate Keystone Database - see /tmp/os_logs/keystone.log"		
		fi
...
### Restart Apache2
	printf " ### Restart Apache2 Service"
		if service apache2 restart >> /tmp/os_logs/keystone.log 2>&1; then
			...
			printf $red "  ERROR --> Could not restart Keystone Service - see /tmp/os_logs/keystone.log"			
		fi
### Remove Dummy Database
	...
	printf " ### Create Keystone Domains \n"
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 domain create default --description "Default" >> /tmp/os_logs/keystone.log 2>&1; 			then printf " --> Created Keystone Domain Heat \n"; 	else printf "  ERROR  --> Could not create Keystone Domain Heat - see /tmp/os_logs/keystone.log\n"; fi
	...
	printf " ### Create Roles \n"
		...
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 role create HeatStackUser >> /tmp/os_logs/keystone.log 2>&1; 	then printf " -->  Created Role HeatStackUser\n"; 	else printf " ERROR --> Could not create role HeatStackUser \n"; fi
	printf " ### Create Users \n"		
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 user create --domain default --project service --password Password123! admin >> /tmp/os_logs/keystone.log 2>&1; 		then printf " -->  Created User  admin\n"; 		else printf " ERROR --> Could not create User admin \n"; fi
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 user create --domain default --project service --password Password123! glance >> /tmp/os_logs/keystone.log 2>&1; 		then printf " -->  Created User  glance\n"; 		else printf " ERROR --> Could not create User glance \n"; fi
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 user create --domain default --project service --password Password123! nova >> /tmp/os_logs/keystone.log 2>&1; 		then printf " -->  Created User  nova\n"; 		else printf " ERROR --> Could not create User nova \n"; fi
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 user create --domain default --project service --password Password123! neutron >> /tmp/os_logs/keystone.log 2>&1; 	then printf " -->  Created User  neutron\n"; 	else printf " ERROR --> Could not create User neutron \n"; fi
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 user create --domain default --project service --password Password123! cinder >> /tmp/os_logs/keystone.log 2>&1; 		then printf " -->  Created User  cinder\n"; 		else printf " ERROR --> Could not create User cinder \n"; fi
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 user create --domain default --project service --password Password123! heat >> /tmp/os_logs/keystone.log 2>&1; 	then printf " -->  Created User heat\n"; 	else printf " ERROR --> Could not create User heat \n"; fi
		# In Domain Heat
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 user create --domain heat --password Password123! HeatDomainAdmin  >> /tmp/os_logs/keystone.log 2>&1; 	then printf " -->  Created User HeatDomainAdmin in Domain heat\n"; 	else printf " ERROR --> Could not create User HeatDomainAdmin in Domain heat\n"; fi
	printf " ### Map User to Role and Project \n"
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 role add --project admin --user admin admin >> /tmp/os_logs/keystone.log 2>&1; 		then printf " -->  Map user admin to project admin and role admin \n"; 		else printf " ERROR --> Could not map user admin to project admin and role admin  \n"; fi
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 role add --project admin --user admin HeatStackOwner >> /tmp/os_logs/keystone.log 2>&1; 		then printf " -->  Map user admin to project admin and role HeatStackOwner \n"; 		else printf " ERROR --> Could not map user admin to project admin and role HeatStackOwner  \n"; fi
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 role add --project service --user glance admin >> /tmp/os_logs/keystone.log 2>&1; 	then printf " -->  Map user glance to project service and role admin \n"; 	else printf " ERROR --> Could not map user glance to project service and role admin  \n"; fi
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 role add --project service --user nova admin >> /tmp/os_logs/keystone.log 2>&1; 		then printf " -->  Map user nova to project service and role admin \n"; 		else printf " ERROR --> Could not map user nova to project service and role admin  \n"; fi
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 role add --project service --user neutron admin >> /tmp/os_logs/keystone.log 2>&1; 	then printf " -->  Map user neutron to project service and role admin \n"; 	else printf " ERROR --> Could not map user neutron to project service and role admin  \n"; fi
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 role add --project service --user cinder admin >> /tmp/os_logs/keystone.log 2>&1; 	then printf " -->  Map user cinder to project service and role admin \n"; 		else printf " ERROR --> Could not map user cinder to project service and role admin  \n"; fi
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 role add --project service --user heat admin >> /tmp/os_logs/keystone.log 2>&1; 	then printf " -->  Map user heat to project service and role admin \n"; 		else printf " ERROR --> Could not map user heat to project service and role admin  \n"; fi
		# In Domain heat
		if openstack --os-url $os_url --os-token=$os_token --os-identity-api-version 3 role add --domain heat --user HeatDomainAdmin admin >> /tmp/os_logs/keystone.log 2>&1; 	then printf " -->  Map user HeatDomainAdmin to role admin in Domain Heat \n"; 		else printf " ERROR --> Could not map user HeatDomainAdmin to role admin in Domain Heat\n"; fi
