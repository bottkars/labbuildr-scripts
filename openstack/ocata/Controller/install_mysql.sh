...
if (mysql -u root --password='Password123!' -e "CREATE DATABASE nova_cell0;") >> $LOGFILE 2>&1; 			then printf " --> SUCCESSFUL nova_cell0 Database created\n"; 		else printf " --> ERROR Could not create nova_cell0 Database - see $LOGFILE \n" | tee -a $LOGFILE; fi
...
then printf " --> SUCCESSFUL entitled user nova on Database nova \n"; 	else printf " --> ERROR Could not entitled user nova on Database nova - see $LOGFILE \n" | tee -a $LOGFILE; fi	

if (mysql -u root --password='Password123!' -e "GRANT ALL PRIVILEGES ON nova_cell0.* TO 'nova'@'localhost' IDENTIFIED BY 'Password123\!';" &&
	mysql -u root --password='Password123!' -e "GRANT ALL PRIVILEGES ON nova_cell0.* TO 'nova'@'%' IDENTIFIED BY 'Password123\!';") >> $LOGFILE 2>&1
then printf " --> SUCCESSFUL entitled user nova on Database nova \n"; 	else printf " --> ERROR Could not entitled user nova on Database nova - see $LOGFILE \n" | tee -a $LOGFILE; fi	
